...
import BackgroundShapes from "@/components/custom/eLearning/BackgroundShapes";
...
import StudentCourseList from "@/components/custom/eLearning/StudentCourseList";
...
const StudentDashboard: FunctionComponent<StudentDashboardProps> = ({
  navigation,
}) => {
  ...
  const onRefresh = useCallback(() => {
    ...
    Promise.all([fetchUserData(), fetchVariations(), fetchCourses()]).finally(
      () => setRefreshing(false)
    );
  }, [user]);
      ...
      Alert.alert(
        ...
        [
          ...
          })),
          {
            ...
            onPress: () => {
              setModule("eLearning");
              setDashboard("eLearning Dashboard");
              navigation.reset({
                index: 0,
                routes: [
                  { name: "StudentHomeNavigator" as keyof RootStackParamList },
                ],
              });
            },
          },
  ...
  return (
    <Screen backgroundColor={Colors.app.screen}>
      ...
      >
        ...
        <Balance balanceAmount={user?.data.wallet.balance} />
        ...
        <View style={styles.bottomContentsWrapper}>
          ...
          >
            <View style={styles.bottomContentsTitleContainer}>
              <AppText style={styles.bottomContentsTitle}>All Courses</AppText>
              <TouchableOpacity
                onPress={() => navigation.navigate(routes.COURSES_NAVIGATOR)}
              >
                <AppText style={styles.viewAll}>View all</AppText>
              </TouchableOpacity>
            </View>
            <StudentCourseList courses={courses} navigation={navigation} />
            <View style={styles.bottomContentsTitleContainer}>
            ...
            <View style={styles.bottomContentsTitleContainer}>
              <AppText style={styles.bottomContentsTitle}>New Courses</AppText>
            </View>
            <GeneralCourseList
              courses={(courses as CourseType[])
                .filter((course) => course.created_at) // remove undefined dates
                .sort(
                  (a, b) =>
