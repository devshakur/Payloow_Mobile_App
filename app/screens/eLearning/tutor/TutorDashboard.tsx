...
const TutorDashboard: FunctionComponent<TutorDashboardProps> = ({
  navigation,
}) => {
  ...
  const onRefresh = useCallback(() => {
    ...
    Promise.all([fetchUserData(), fetchVariations()]).finally(() => {
      setRefreshing(false);
      if (user?.data.elearningRole === null) {
        setVisible(true);
      }
    });
  }, [user]);
          ...
          })),
          {
            ...
            onPress: () => {
              ...
              navigation.reset({
                index: 0,
                routes: [
                  { name: "TutorHomeNavigator" as keyof RootStackParamList },
                ],
              });
            },
          },
