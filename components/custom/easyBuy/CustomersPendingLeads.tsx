...
interface CustomersPendingLeadsProps {
  screen?: undefined;
}
...
const CustomersPendingLeads: FunctionComponent<CustomersPendingLeadsProps> = ({
  screen,
}) => {
  const [customers] = useState(generateTransactions("customers"));
  const [pending] = useState(generateTransactions("pending"));
  const [leads] = useState(generateTransactions("leads"));
  const [currentPage, setCurrentPage] = useState(1);

  const transactions =
    screen === "customers" ? customers : screen === "pending" ? pending : leads;
...
export default CustomersPendingLeads;
